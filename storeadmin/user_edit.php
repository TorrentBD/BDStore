...
if (isset($_POST['name'])) {
	...
	$sql = mysqli_query($con,"UPDATE customer SET name='$name',login='$login',mobile='$mobile',securityanswer='$securityanswer',email='$email',address='$address',city='$city', state='$state',pin='$pin' WHERE id='$pid'");
...
if (isset($_GET['pid'])) {
	$targetID = $_GET['pid'];
    $sql = mysqli_query($con,"SELECT * FROM customer WHERE id='$targetID' LIMIT 1");
