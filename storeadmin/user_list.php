...
if (isset($_GET['yesdelete'])) {
	...
	$sql = mysqli_query($con,"DELETE FROM customer WHERE id='$id_to_delete' LIMIT 1") or die (mysqli_error());
...
if (isset($_POST['name'])) {
	...
	$security = mysqli_real_escape_string($_POST['security']);
	$securityanswer = mysqli_real_escape_string($_POST['answer']);
	$email = mysqli_real_escape_string($_POST['email']);
	$address = mysqli_real_escape_string($_POST['address']);
	$city = mysqli_real_escape_string($_POST['city']);
	$state = mysqli_real_escape_string($_POST['state']);
	$pin = mysqli_real_escape_string($_POST['pin']);
	if($password != $cpassword){
		echo 'Your passwords do not match., <a href="user_registration.php">Refill here</a>';
		exit();
	}
	$sql = mysqli_query($con,"SELECT id FROM customer WHERE mobile='$mobile' LIMIT 1");
	...
	$sql = mysqli_query($con,"INSERT INTO customer (login, password, name, mobile, security, securityanswer, email, address, city, state, pin) 
        VALUES('$login', '$password','$name','$mobile','$security','$securityanswer','$email','$address','$city','$state','$pin')") or die (mysqli_error());
